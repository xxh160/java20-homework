...
import huluwa.characters.Grandpa;
import huluwa.tools.*;
...
public class CollectionSort {
    ...
    public static void main(String[] args) {
        ...
        grandpa.setSorter(new BubbleSorter());
        grandpa.sort(queue, CalabashBrother::compareTo);
        System.out.println("Ascending sort result:");
        queue.forEach(CalabashBrother::numberOff);
        // 用字典序逆序重新排序，这次用快速排序
        grandpa.setSorter(new QuickSorter());
        grandpa.sort(queue, new DescendingComparator());
        System.out.println("Descending sort result:");
        queue.forEach(CalabashBrother::numberOff);
        // 用乱序排序，仍然用快速排序
        grandpa.sort(queue, new RandomComparator());
        ...
        femaleQueue.get(femaleQueue.size() - 1).quickSort(femaleQueue, 0, femaleQueue.size() - 1,
                femaleQueue.get(0), false, CalabashBrother::compareTo);
