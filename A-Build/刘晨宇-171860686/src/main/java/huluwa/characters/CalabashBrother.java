...
public class CalabashBrother extends Character implements Comparable<CalabashBrother>,
    ...
    public void bubbleSort(List<CalabashBrother> list, int thisIndex, int lastIndex,
                           Comparator<? super CalabashBrother> comparator) {
        ...
        if(thisIndex < lastIndex) {
            ...
            nextBrother.bubbleSort(list, thisIndex + 1, lastIndex, comparator);
        } else if(lastIndex > 0) {
            list.get(0).bubbleSort(list, 0, lastIndex - 1, comparator);
        }
    }

    // Choreography形式的快速排序，与其他葫芦娃协同完成，不依赖于Sorter
    public void quickSort(List<CalabashBrother> list, int thisIndex, CalabashBrother base,
        ...
        if(thisIndex + (forward ? 1 : -1) == baseIndex) {
            ...
            if(baseIndex - 1 > 0) {
                ...
                leftSuccessor.quickSort(list.subList(0, baseIndex), baseIndex - 1, leftBase,
                        0, false, comparator);
            }
            if(baseIndex + 1 < list.size() - 1) {
                ...
                rightSuccessor.quickSort(list.subList(baseIndex + 1, list.size()),
                        list.size() - baseIndex - 2, rightBase, 0, false, comparator);
            }
        } else {
            // 通过与基准葫芦娃进行比较，结合当前排序的进行方向，判断是否交换位置
            ...
            if(comparator.compare(this, base) < 0 ^ forward) {
                swap(list, base, thisIndex, baseIndex);
                int successorIndex = baseIndex + (forward ? -1 : 1);
                CalabashBrother successor = list.get(successorIndex);
                baseIndex = thisIndex;
                successor.quickSort(list, successorIndex, base, baseIndex, !forward, comparator);
            } else {
                int successorIndex = thisIndex + (forward ? 1 : -1);
                CalabashBrother successor = list.get(successorIndex);
                successor.quickSort(list, successorIndex, base, baseIndex, forward, comparator);
            }
        }
