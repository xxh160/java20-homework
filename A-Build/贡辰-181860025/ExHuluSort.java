...
class orchestration extends hulusort
{
    public orchestration(int size,int[] seed)
    {
        ...
        {
            for (int i=0;i<size;i++)
                if (seed[i]>=0 && seed[i]<=6 && v[seed[i]]==0)
                    v[seed[i]]++;
                else
                {
                    //System.out.println("initial error!");
                    throw(new commonException("initial error!"));
                }
        }
        catch(commonException ex)
        {
            //System.out.println("initial error!");
            ExceptionUtils exutil=new ExceptionUtils();
            System.out.println(exutil.getStackTrace(ex));
        }
