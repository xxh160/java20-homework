...
public class Family<T extends Creature> implements Iterable<T> {
    ...
    ArrayList<String> get_name_list() {
        int length = family.size();
        for (int i = 0; i < length; i++) {
            name_list.add(family.get(i).get_name());
        }
        return name_list;
    }

    String get_random_name() {
        // RandomStringUtils rando=new RandomStringUtils();
        // String str="abcdefghijklmnopqrstuvwxyz";
        /*
         * long time_now=System.currentTimeMillis(); Random ran_num=new
         * Random(time_now);
         */
        /*
         * String name_buffer=""; int length_name=rng.nextInt(10); for(int
         * i=0;i<length_name;i++) { int index=rng.nextInt(26);
         * name_buffer=name_buffer+str.charAt(index); } return name_buffer;
         */
    ...
    public void display_reverse(Family<T> my_family) {
        System.out.println();
        CalaBash_Comparetor_normal com = new CalaBash_Comparetor_normal();
        Ordering<Creature> list_rever = Ordering.from(com);
        Collections.sort(family, list_rever);
        System.out.println("逆序：");
        /*
         * while(it.hasNext()) { Calabash p=it.next();
    ...
    public Iterable<T> randomized() {
        return new Iterable<T>() {
            public Iterator<T> iterator() {
                List<T> shuffled = new ArrayList<T>(family);
    ...
    public void team(Iterator<T> it) {
        ...
        for (T s : team_list_girl)
            System.out.print(s.get_name() + "    ");
        // ImmutablePair<ArrayList<T>, ArrayList<T>> result = new
        // ImmutablePair<ArrayList<T>, ArrayList<T>>(team_list_boy,
        // team_list_girl);
        // return result;
