...
class huluwa implements Comparable<huluwa>{
    ...
    public int compareTo(huluwa a){
        if(name.compareTo(a.name)<0) return -1;
        else if(name.compareTo(a.name)>0) return 1;
        return 0;
    }
    ...
    public huluwa(){name = itsname();_sex = itssex();}
    public String get_name(){return name;}
    public sex get_sex(){return _sex;};
}
class yeyecomparator implements Comparator<huluwa>{
    @Override
